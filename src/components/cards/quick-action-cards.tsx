...
export default function QuickActionCard({ title, image, slug, className }: QuickActionCardProps) {
  return (
    <Link
      href={`/${slug}`}
      className={cn(
        "group relative block aspect-[16/10] overflow-hidden rounded-2xl bg-muted transition-all hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2",
        className
      )}
    >
      {/* Background Image */}
